...
class HiddenLayer(object):
    def __init__(self, input_size, output_size, an_id):
        ...
        W, b = AnnHelper.get_weight_and_bias(input_size, output_size)
...
class ANN(object):
    ...
    def fit(self, data, targets, learning_rate=10e-7, mu=0.99, decay=0.999, reg=10e-3, epochs=400, batch_sz=100, show_fig=false):
        ...
        x_train, x_test, y_train, y_test = train_test_split(data ,targets, test_size=0.2, random_state=44)
        ...
        for output_index in self.hidden_layer_number:
            layer = HiddenLayer(input_index, output_index, count)
            self.hidden_layers.append(h)
            input_index = output_index
            count += 1
        W, b = AnnHelper.get_weight_and_bias(input_index, target_lenght)
        self.W = tf.Variable(W.astype(np.float32))
        self.b = tf.Variable(b.astype(np.float32))
        ...
        tfY = tf.placeholder(tf.float32, shape=(None, target_lenght), name='tfY')
        activation = self.forward(tfX)

        reg_cost = reg*sum([tf.nn.l2_loss(p) for p in self.params])
        cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(activation, tfY)) + reg_cost
